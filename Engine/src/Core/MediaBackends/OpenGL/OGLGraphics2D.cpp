...
OGLRenderer2D::OGLRenderer2D(Window& window)
{
    ...
    resize_delegate->Bind(this, &OGLRenderer2D::UpdateViewPort);
...
void OGLRenderer2D::AddShader(const std::string vertex_path, const std::string fragment_path)
{
...
void OGLRenderer2D::SetMVPBlock()
{
...
void OGLRenderer2D::UpdateViewPort(EWindowResized* e)
{
    glViewport(0, 0, (uint) e->new_size.x, (uint) e->new_size.y);
}

void OGLRenderer2D::ClearDrawBuffers()
{
...
void OGLRenderer2D::DrawText(std::vector<uint> current_text_textures)
{
...
bool OGLRenderer2D::PrepareDraw()
{
    if(!m_initialized)
        return false;
...
void OGLRenderer2D::Draw(uint model_buffer_size, uint model_instance_amount, uint buffer_offset, uint mvp_index)
{
    //Draw Objects
